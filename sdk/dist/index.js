...
__export(index_exports, {
  OrbitalSDK: () => OrbitalSDK,
  aprBpsKinked: () => aprBpsKinked,
  calculateAssetDue: () => calculateAssetDue,
  calculateLSTDue: () => calculateLSTDue,
  calculateLSTPrice: () => calculateLSTPrice,
  createDepositBoxName: () => createDepositBoxName,
  createLoanBoxName: () => createLoanBoxName,
  createOraclePriceBoxName: () => createOraclePriceBoxName,
  currentAprBps: () => currentAprBps,
  decodeDepositRecord: () => decodeDepositRecord,
  decodeLoanRecord: () => decodeLoanRecord,
  decodeOraclePrice: () => decodeOraclePrice,
  fetchMarketInfo: () => fetchMarketInfo,
  fetchMarketList: () => fetchMarketList,
  getApplicationGlobalState: () => getApplicationGlobalState,
  getBoxValue: () => getBoxValue,
  microToStandard: () => microToStandard,
  standardToMicro: () => standardToMicro,
  utilNormBps: () => utilNormBps
});
  ...
  for (const item of globalState) {
    ...
    if (value.type === 1) {
      ...
      if (value.bytes && Buffer.from(value.bytes, "base64").length === 32) {
        try {
          state[key] = import_algosdk.default.encodeAddress(Buffer.from(value.bytes, "base64"));
        } catch {
          state[key] = Buffer.from(value.bytes, "base64");
        }
      }
...
function decodeDepositRecord(boxValue) {
  const depositRecordType = new import_algosdk.default.ABITupleType([
    new import_algosdk.default.ABIUintType(64),
    // assetId
    new import_algosdk.default.ABIUintType(64)
    // depositAmount
  ]);
...
function decodeLoanRecord(boxValue) {
  const loanRecordType = new import_algosdk.default.ABITupleType([
    new import_algosdk.default.ABIAddressType(),
    // borrowerAddress
    new import_algosdk.default.ABIUintType(64),
    // collateralTokenId
    new import_algosdk.default.ABIUintType(64),
    // collateralAmount
    new import_algosdk.default.ABITupleType([
      new import_algosdk.default.ABIUintType(64),
      // lastDebtChange.amount
      new import_algosdk.default.ABIUintType(8),
  ...
  const borrowerAddress = typeof borrowerAddressRaw === "string" ? borrowerAddressRaw : import_algosdk.default.encodeAddress(borrowerAddressRaw);
...
function createDepositBoxName(userAddress, assetId) {
  const depositKeyType = new import_algosdk.default.ABITupleType([
    new import_algosdk.default.ABIAddressType(),
    new import_algosdk.default.ABIUintType(64)
    // assetId
  ]);
  ...
  const encodedKey = depositKeyType.encode([
    import_algosdk.default.decodeAddress(userAddress).publicKey,
    assetId
  ]);
...
function createLoanBoxName(userAddress) {
  ...
  const addressBytes = import_algosdk.default.decodeAddress(userAddress).publicKey;
  return new Uint8Array([...prefix, ...addressBytes]);
}
function decodeOraclePrice(boxValue) {
  const oraclePriceType = new import_algosdk.default.ABITupleType([
    new import_algosdk.default.ABIUintType(64),
    // assetId
    new import_algosdk.default.ABIUintType(64),
    // price
