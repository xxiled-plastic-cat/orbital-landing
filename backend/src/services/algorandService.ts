...
import { getOrSetCache, CacheKeys, CacheTTL } from './cacheService.js';
...
export async function getApplicationGlobalState(appId: number): Promise<Record<string, any>> {
  const cacheKey = `${CacheKeys.APP_GLOBAL_STATE}${appId}`;
...
export async function getTokenPriceFromOracle(
  ...
  const cacheKey = `${CacheKeys.ORACLE_PRICE}${oracleAppId}:${assetId}`;
...
export async function getAssetInfo(assetId: number): Promise<{
  ...
  const cacheKey = `${CacheKeys.ASSET_INFO}${assetId}`;
